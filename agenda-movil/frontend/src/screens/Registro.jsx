...
export default function Registro() {
  ...
  const formatRut = (value) => {
    ...
    if (!/^\d{0,8}[\dkK]?$/.test(cleanValue)) return rut; // Retorna el valor actual si no es válido
  ...
  const getFileSizeInKB = (base64) => {
    ...
    try {
      const response = await fetch('https://taller-3.onrender.com/api/register', {
        method: 'POST',
        body: formData,
      });
